 ...
 */
/*
 * $Id: functions.c,v 1.1 2001/01/15 03:14:10 wenk Exp $ 
 */
...
extern int skill_check(dbref,int,int);
...
/* VALDAR, added skill check*/

FUNCTION(fun_skill_check)
{
	fval(buff, bufc, (float)skill_check(match_thing(player,fargs[0]),atoi(fargs[1]),atoi(fargs[2])));
}
...
{"SKILL_CHECK",	fun_skill_check,3,  0,		CA_WIZARD},
...
CF_HAND(cf_func_access)
{
	...
	for (fp = flist; fp->name; fp++) {
		if (!string_compare(fp->name, str)) {
			return (cf_modify_bits((long *)&fp->perms, ap, extra,
					       player, cmd));
		}
	}
	for (ufp = ufun_head; ufp; ufp = ufp->next) {
		if (!string_compare(ufp->name, str)) {
			return (cf_modify_bits((long *)&ufp->perms, ap, extra,
					       player, cmd));
		}
	}
