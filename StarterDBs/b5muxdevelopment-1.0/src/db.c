 ...
 */
/*
 * $Id: db.c,v 1.1 2001/01/15 03:14:10 wenk Exp $ 
 */
...
ATTR attr[] =
{
	...
	{"Lastsite", A_LASTSITE, AF_ODARK | AF_NOPROG | AF_NOCMD | AF_WIZARD,
	 NULL},
   ...
   {"ReceiveLock", A_LRECEIVE, AF_ODARK | AF_NOPROG | AF_NOCMD | AF_IS_LOCK,
	...
	{"VZ", A_VA + 25, AF_ODARK, NULL},
        {"WEAR", WEAR, AF_DARK|AF_WIZARD, NULL},
        {"WIELD", WIELD, AF_DARK|AF_WIZARD, NULL},
        {"ITEM", ITEM_A, AF_DARK|AF_WIZARD, NULL},
        {"SKILLS", SKILL_A, AF_MDARK|AF_WIZARD, NULL},
        {"CARGO", CARGO, AF_MDARK|AF_WIZARD, NULL},
        {"MCARGO", MCARGO, AF_MDARK|AF_WIZARD, NULL},
        {"SIZE", SIZE, AF_MDARK|AF_WIZARD, NULL},
        {"ADVANS", ADVAN_A, AF_MDARK|AF_WIZARD, NULL},
        {"ATTRIBS", ATRIBS_A, AF_MDARK|AF_WIZARD, NULL},
        {"COMMODITIES", COMMOD_A, AF_MDARK|AF_WIZARD, NULL},
        {"RANK", RANK_A, AF_ODARK|AF_WIZARD, NULL},
        {"FACTION", FACTION_A, AF_ODARK|AF_WIZARD, NULL},
        {"PERCEPCHA", PERCHA_A, AF_ODARK|AF_WIZARD, NULL},
        {"PERCEPVAL", PERCEPT_A, AF_ODARK|AF_WIZARD, NULL},
        {"RACE", RACE_A, AF_ODARK|AF_WIZARD, NULL},
        {"XCOORD", XC_A, AF_MDARK|AF_WIZARD, NULL},
        {"YCOORD", YC_A, AF_MDARK|AF_WIZARD, NULL},
        {"ZCOORD", ZC_A, AF_MDARK|AF_WIZARD, NULL},
        {"VMONEY", MONEY_A, AF_MDARK|AF_WIZARD, NULL},
        {"LANGUAGE", LANG_A, AF_MDARK|AF_WIZARD, NULL},
        {"STRUCPTS", STRUC_A, AF_MDARK|AF_WIZARD, NULL},
        {"CLASS", CLASS_A, AF_ODARK|AF_WIZARD, NULL},
        {"TITLE", TITLE_A, AF_ODARK|AF_WIZARD, NULL},
        {"SHIPNUM", SHIP_A, AF_ODARK|AF_WIZARD, NULL},
        {"COMPUTER", COMPUTER_A, AF_MDARK|AF_WIZARD, NULL},
        {"TIMESKILLS", TSKILL_A, AF_MDARK|AF_WIZARD, NULL},
        {"TIMEMED", MSKILL_A, AF_MDARK|AF_WIZARD, NULL},
...
char *buff;
{
	...
	if (!db[thing].ahead) {
        ...
        * * RADIX_COMPRESSION  
        */
		list[0].checksum = make_checksum(text, list[0].size);
		found = 1;
	} else {
        ...
        * * RADIX_COMPRESSION  
        */
				list[mid].checksum = make_checksum(text, list[mid].size);
				found = 1;
				break;
			} else if (list[mid].number > atr) {
				hi = mid - 1;
        ...
        * * RADIX_COMPRESSION  
        */
			list[lo].checksum = make_checksum(text, list[lo].size);
			db[thing].at_count++;
			db[thing].ahead = list;
		}
	}
...
int atr;
{
	...
	while (lo <= hi) {
		...
		if (list[mid].number == atr) {
			...
			/* Check to make sure checksum matches */
			
			if (!corrupt && (make_checksum(list[mid].data, list[mid].size) != list[mid].checksum)) {
				STARTLOG(LOG_ALWAYS, "BUG", "CRUPT")
					log_text((char *)tprintf("Database is corrupt, object %d. Exiting.", (int)thing));
				ENDLOG
