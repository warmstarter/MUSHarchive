 ...
 */
/*
 * $Id: rob.c,v 1.1 2001/01/15 03:14:10 wenk Exp $ 
 */
...
notify(player,"This command is disabled");
return;
...
char *what;
{
	...
	if(Flags3(thing)&IC && !(Flags3(recipient)&&IC)) {
		notify(giver,"You cant give an IC object to a non-IC thing.");
		return;
	}
	if(Check_Enc(recipient,thing)==-1) {
		notify(giver,"That thing cant hold any more.");
		return;
	}
