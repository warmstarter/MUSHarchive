...
if(i!=2) {
		notify(player,"Usage: ACCESS <ACCOUNT> <CODE>");
		return;
	}
...
i=xcode_parse(buffer,fargs,2);
if(i!=2) {
		notify(player,"Usage: DEPOSIT <AMOUNT in specified currency> <CURRENCY>");
		return;
	}
...
i=xcode_parse(buffer,fargs,2);
if(i!=2) {
		notify(player,"Usage: WITHDRAW <AMOUNT in monetary units> <CURRENCY>");
		return;
	}
if(atoi(vget_a(area,RANK_A))!=player) {
		notify(player,"You are not accessing any account");
...
static char *rcsbankc="$Id: bank.c,v 1.1 2001/01/15 03:23:15 wenk Exp $";
