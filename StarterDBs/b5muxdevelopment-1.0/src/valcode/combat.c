...
Combat *playe;
{
float helth,bld,intu,fat,oldfat;
int play;
	...
	if(Flags3(play) & UNCON) {
		if (fat >= 0.5) {
			notify_all_from_inside(where_is(play),1,tprintf("%s has woken up.",Name(play)));
			s_Flags3(play, Flags3(play) & ~UNCON);
		}
	}
}
...
change_flist(player,7,helth,ATRIBS_A);
    if (helth <= 0.0) { /* uncon code */
        do_rest(player,player,0);
        s_Flags3(player, Flags3(player) | UNCON);
        notify_all_from_inside(where_is(player),1,tprintf("%s has fallen unconscious!",Name(player)));
    }
}
...
static char *rcscombatc="$Id: combat.c,v 1.1 2001/01/15 03:23:15 wenk Exp $";
