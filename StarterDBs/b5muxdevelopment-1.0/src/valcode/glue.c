...
#include "./vmspace/header.h"
#include "./vmspace/mech/mech.h"
#include "./vmspace/mech/mechrep.h"
#include "./vmspace/mech/comp.h"
#include "./vmspace/mech/debug.h"
#include "./vmspace/vminterface.h"
#include "econ.h"
#include "relays.h"
#include "mbankinterface.h"
...
/*#define Power(x,y) 	has_power(x,1,y)*/

/*tmp fix since the Power stuff form muse and MUX arent exact same - VAL*/
#define Power(x,y) 	Builder(x)
#define log_important(x) printf(x)


void newfreespace();  /*A USELESS func since i dont utilize XCODES dynamic
			Memory utilities*/
...
extern int string_compare2(char *,char *);
...
int first, last, current;
  ...
  if(strncmp(command, "HELP", 4)==0) {
    ...
    if (!dir)
    {
  	buff=vfetch_attribute(location, "XTYPE");
	if(strcmp(buff,"CREATE1")!=0 && strcmp(buff,"CREATE2")!=0 && strcmp(buff,"CREATE3")!=0) {
		if(!IsIcReg(player))
			return 1;
	}
...
int type;
  ...
  for(i=0; i<mudstate.db_top; i++) {
    if(Flags2(i) & CONSOLE) {
	  ...
	  db[i].flags &= ~CONSOLE; /* Reset the flag */
    }
  }
...
int type;
  ...
  typeAttr=vfetch_attribute(key, "XTYPE");
  if(!typeAttr) {
    ...
    db[key].flags &= ~CONSOLE; /* Reset the flag */
    return;
  }
...
char *typeAttr;
  ...
  if(typeAttr) {
    for(i=0; i<NUM_SPECIAL_OBJECTS; i++) {
      if(!strcmp(SpecialObjects[i].type, typeAttr)) 
      {
	returnValue= i;
	break;
      }
...
struct SpecialObjectInstance *temp;
char *typeAttr;
/*ATRDEF *k;    Ive NO idea why this used to be here*/
    if (key < 0 || key > mudstate.db_top) { 
	return -1;
    }
    typeAttr=vfetch_attribute(key, "XTYPE");
    if(!typeAttr) {
    ...
    {
	temp->data = data;
	vput_attribute(key,typeAttr,SpecialObjects[which].type);
	return 1;
    }
    return 0;
}
...
struct CommandsStruct *temp;
	...
	for (i=0; i<nc-1; i++)
	{
	    if (string_compare2(SortedCommands[which][i]->name,
...
static char *rcsgluec="$Id: glue.c,v 1.1 2001/01/15 03:23:16 wenk Exp $";
