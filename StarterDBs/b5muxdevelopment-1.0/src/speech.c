 ...
 */
/*
 * $Id: speech.c,v 1.1 2001/01/15 03:14:10 wenk Exp $ 
 */
...
char *scramble( const char *argument, int modifier )
{
    ...
    modifier %= (random()%220+80); /* Bitvectors get way too large #s */
    for ( position = 0; position < MAX_INPUT_LENGTH; position++ )
    {
    	...
    	else if ( argument[position] >= 'A' && argument[position] <= 'Z' )
	    {
	    	...
	    	conversion = (random()%10+conversion-5);
	    	while ( conversion > 25 )
	    		conversion -= 26;
	    	while ( conversion < 0 )
	    		conversion += 26;
	    ...
	    else if ( argument[position] >= '0' && argument[position] <= '9' )
	    {
	    	...
	    	conversion = (random()%4+conversion-2);
...
char *message;
{
	...
	case SAY_SAY:
		...
		notify_except3(loc, player, player
		, tprintf("%s says \"%s\"", Name(player), message),message);


/*		notify_except(loc, player, target, tprintf("%s says \"%s\"", 
			Name(target), message));
*/
		break;
	case SAY_POSE:
...
if(!Wizard(player) && (Flags3(player) & IC) && !(Flags2(player)&STAFF) ) {
	notify(player,"You can not do that when IC");
	return;
	}
...
if(!Wizard(target) && (Flags3(target) & IC)) {
	notify(player,"NOTE: That player is IC and cannot return your page.");
	}
