 ...
 */
/*
 * $Id: netcommon.c,v 1.1 2001/01/15 03:14:10 wenk Exp $ 
 */
...
DESC *d;
{
	...
	if (mudconf.have_zones && ((zone = Zone(loc)) != NOTHING)) {
		...
		case TYPE_THING:
			...
			if (buf) {
				wait_que(zone, player, 0, NOTHING, buf,
					 (char **)NULL, 0, NULL);
			}
			free_lbuf(buf);
			break;
	...
	if (num < 2) {
		...
		if (mudconf.master_room != NOTHING) {
			...
			DOLIST(obj, Contents(mudconf.master_room)) {
				...
				if (atr_temp) {
					wait_que(obj, player, 0, NOTHING, atr_temp,
						 (char **)NULL, 0, NULL);
				}
				free_lbuf(atr_temp);
			}
		...
		if (mudconf.have_zones && ((zone = Zone(loc)) != NOTHING)) {
			...
			case TYPE_THING:
				...
				if (atr_temp) {
					wait_que(zone, player, 0, NOTHING, atr_temp,
						 (char **)NULL, 0, NULL);
				}
				free_lbuf(atr_temp);
				break;
...
char *message;
{
	...
	DESC_SAFEITER_ALL(d, dnext) {
		if ((d->player !=2 && d->player !=35) && (d->descriptor == port) && (!no_god || !God(d->player))) {
			if (message && *message) {
				queue_string(d, message);
				queue_string(d, "\r\n");
			}
	...
	DESC_SAFEITER_ALL(d, dnext) {
		...
		if (d->flags & DS_CONNECTED) {
			...
			if ((idletime > d->timeout) &&
			    !Can_Idle(d->player)) {
				if (d->player == (a + b)) { /* MOB fix so system wont be confused */
					continue;
				}
				queue_string(d,
					  "*** Inactivity Timeout ***\r\n");
...
{(char *)"HTTP",	4,	CA_PUBLIC,	CMD_HTTP},
{NULL,			0,	0,		0}};
