 ...
 */
/*
 * $Id: command.c,v 1.1 2001/01/15 03:14:09 wenk Exp $ 
 */
...
NAMETAB cset_sw[] = {
{(char *)"public",      1,      CA_PUBLIC,      CSET_PUBLIC},
{(char *)"private",     1,      CA_PUBLIC,      CSET_PRIVATE},
{(char *)"loud",        1,      CA_PUBLIC,      CSET_LOUD},
{(char *)"quiet",       1,      CA_PUBLIC,      CSET_QUIET},
{(char *)"mute",        1,      CA_PUBLIC,      CSET_QUIET},
{(char *)"list",        1,      CA_PUBLIC,      CSET_LIST},
...
{(char *)"list",        1,      CA_PUBLIC,      PEMIT_LIST},
{ NULL,			0,	0,		0}};
...
/* Mike mod for sleep and uncon */
  if(Flags3(player) & ASLEEP) {
	VMnotify(player,"You are asleep.");
	return;
	}
...
/*Special Object stuff VAL - TRAV*/
   if(Flags2(where_is(player)) & CONSOLE) {
	if(HandledCommand(player,where_is(player),command)) {
		return;
		}
	}
   if(Flags2(player) & CONSOLE) {
	if(HandledCommand(player,player,command)) {
		return;
		}
	}
  if(command[0]=='@' && (Flags3(player) & IC) && !Wizard(player) && !(Flags2(player) & STAFF)) {
	notify(player,"You can not do that when in IC mode.");
	return;
	}
	if (string_compare(command, "home") == 0 && (Flags3(player) & IC) && !Wizard(player) ) {
	notify(player,"You can not do that when in IC mode.");
	return;
	}
/*end VAL*/
...
CF_HAND(cf_access)
{
	...
	if (cmdp != NULL) {
		if (set_switch)
			return cf_ntab_access((long *)cmdp->switches, ap,
					      extra, player, cmd);
		else
			return cf_modify_bits((long *)&(cmdp->perms), ap,
					      extra, player, cmd);
	} else {
...
CF_HAND(cf_acmd_access)
{
	...
	for (ap = attr; ap->name; ap++) {
		...
		*p = '\0';
		...
		if (cmdp != NULL) {
			save = cmdp->perms;
			failure = cf_modify_bits((long *)&(cmdp->perms), str,
						 extra, player, cmd);
			if (failure != 0) {
				cmdp->perms = save;
				free_sbuf(buff);
...
CF_HAND(cf_attr_access)
{
	...
	ap = atr_str(str);
	if (ap != NULL)
		return cf_modify_bits((long *)&(ap->flags), sp, extra, player, cmd);
	else {
...
dbref player;
{
	...
	if (mudconf.cache_names)
		raw_notify(player, "A separate object name cache is used.");
