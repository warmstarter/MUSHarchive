 ...
 * 
 * $Id: udb_ocache.c,v 1.1 2001/01/15 03:14:11 wenk Exp $
 */
...
Obj *obj;
{
	...
	while (lo <= hi) {
#ifndef STANDALONE
	  if (count > 200000) {break; VMnotify(35,"SHIT!");}
	  count++;
#endif
		mid = ((hi - lo) >> 1) + lo;
        ...
        */
{
	...
	if (obj->atrs == ATNULL) {
        ...
        * RADIX_COMPRESSION 
        */
		a[0].checksum = make_checksum((char *)a[0].data, a[0].size);
		return;
	}
	/*
	 * Binary search for the attribute. 
	...
	while (lo <= hi) {
        ...
        * RADIX_COMPRESSION 
        */
			a[mid].checksum = make_checksum((char *)a[mid].data, a[mid].size);
			return;
		} else if (a[mid].attrnum > anam->attrnum) {
			hi = mid - 1;
		} else {
	...
	a[lo].size = ATTR_SIZE(value);
        ...
        * RADIX_COMPRESSION 
        */
	a[lo].checksum = make_checksum((char *)a[lo].data, a[lo].size);
	obj->at_count++;
	obj->atrs = a;
