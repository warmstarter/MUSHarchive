 ...
 */
/*
 * $Id: look.c,v 1.1 2001/01/15 03:14:10 wenk Exp $ 
 */
...
/* VALTRAV*/
	if(QuietIsIcReg(thing) && isPlayer(thing)) {

		tmp=vget_a(thing,WIELD);
		weapon=match_thing(0,tmp);
		if(weapon==NOTHING || weapon==0 || !(Flags2(weapon) & WEAPON)) {
			weapon=-1;
		}

		tmp=vget_a(thing,ITEM_A);
		item=match_thing(0,tmp);
		if(item==NOTHING || item==0 || !(Flags3(item) & ITEM)) {
			item=-1;
		}

		tmp=vget_a(thing,WEAR);
		armor=match_thing(0,tmp);
		if(armor==NOTHING || armor==0 || !(Flags2(armor) & ARMOR)) {
			armor=-1;
		}

		body=read_flist(thing,1,ATRIBS_A);
		health=read_flist(thing,6,ATRIBS_A);
if(body > 0) {
		if(health/(5. * body) >= 1) sprintf(msg,"perfect");
		else if(health/(5. * body) >= .8) sprintf(msg,"great");
		else if(health/(5. * body) >= .5) sprintf(msg,"good");
		else if(health/(5. * body) >= .3) sprintf(msg,"fair");
		else if(health/(5. * body) >= .1) sprintf(msg,"poor");
		else sprintf(msg,"horrible");


		if(body > 8)  notify(player,tprintf("%s appears huge and in %s health.",Name(thing),msg));
		else if(body > 6)  notify(player,tprintf("%s appears large and in %s health.",Name(thing),msg));
		else if(body > 4)  notify(player,tprintf("%s appears average and in %s health.",Name(thing),msg));
		else if(body > 3)  notify(player,tprintf("%s appears small and in %s health.",Name(thing),msg));
		else notify(player,tprintf("%s appears tiny and in %s health.",Name(thing),msg));

	if(weapon!=-1) notify(player,tprintf("%s is wielding a %s.",Name(thing),Name(weapon)));
	if(item!=-1) notify(player,tprintf("%s is carrying a %s.",Name(thing),Name(item)));
	if(armor!=-1) notify(player,tprintf("%s is wearing a %s.",Name(thing),Name(armor)));
	if(Flags3(player) & UNCON) {
		notify(player,tprintf("%s looks asleep.",Name(thing),msg));
	}
...
char *name;
{
	...
	if (!name || !*name) {
		...
		if (Good_obj(thing)) {
			...
			/* perception based look code */
			if (QuietIsIcReg(player)) {
#ifdef SPACEDEBUG
			  VMnotify(35,"did it");
#endif
			  check_perception(player,thing);
			}
		}
		return;
	}
...
void check_perception(dbref player, dbref room) {
  char *tmp,*tmp2;
  int num = 0;
  int res = 0;
  if (Typeof(room) != TYPE_ROOM) {
    #ifdef SPACEDEBUG
    VMnotify(35,"Not a room. %d",room);
    #endif
    return;
  }

  tmp = vget_a(room,PERCHA_A);
  if (tmp == NULL) {
    /* must not be there */ 
    #ifdef SPACEDEBUG
    VMnotify(35,"tmp is null");
    #endif
    return;
  }

  if (strlen(tmp) > 10) {
    /* anything bigger than 10 should get axed. */
    #ifdef SPACEDEBUG
    VMnotify(35,"tmp is greater than 10 charecters");
    #endif
    return;
  }
  ...
  num = 0 + atoi(tmp); 
  if (num > 0) { 
    /* ok, we'll do the perception check now */
    res = skill_check(player,124,num); 
#ifdef SPACEDEBUG    
    VMnotify(35,"Player: %d Skill: %d Num: %d Res: %d",player,124,num,res);
#endif 
    if (res > 0) {
      /* check is successful */ 
      if (res > num+5) { 
	/* they get the whole nine yards.*/
	tmp2 = (char *) vget_a(room,PERCEPT_A);
	if (tmp2 == NULL) {
	  /* Hey! tmp2 shouldn't be null... */ 
	  VMnotify(player, "You perceive something, but the builder hasn't set what you perceive yet."); 
	  return;
	}
	VMnotify(player,"After looking around the room, you perceive: %s about the room.",tmp2);
	return;
      }
      else if (res > num) {
       
	VMnotify(player,"After looking around, you feel that things are out of the ordinary");
	return;
      }
      else if (res > (num-5)) {
	VMnotify(player,"Something tickles the back of your neck.");
	return;
      }
      else {
	VMnotify(player,"The hair on the back of your hands start to tingle.");
	return;
      }
    }
  }
...
#ifdef SPACEDEBUG    
    VMnotify(35,"tmp must not be a number");
#endif
    return;
  }
}
