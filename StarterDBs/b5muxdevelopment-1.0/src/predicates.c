...
/*
 * $Id: predicates.c,v 1.1 2001/01/15 03:14:10 wenk Exp $ 
 */
...
char *message;
{
	...
	wait_que(d->program_data->wait_cause, d->player, 0, NOTHING, cmd, (char **)&message,
...
/*
 * ---------------------------------------------------------------------------
 * * handle_http: Handles external http requests into the MUX.
 * * Address format is appended to URL of muxcgi program, in this format:
 * *    ?<object number>[/attribute[:arg1=arg2=arg3=arg4 etc...]]
 * *
 * * Ex:  "?123" reads the HTML attribute from #123, if HTML is absent then it
 * *    reads the description of the object.
 * *    "?123/foobar" reads the FOOBAR attribute from #123, if either the
 * *    attribute or the object is visual.
 * *    "?123/foobar:Hey there!" reads the FOOBAR attribute from #123,
 * *    substituting 'Hey there!' for %0 in the attribute.
 */

void handle_http(d, arg)
DESC *d;
char *arg;
{
	char *object, *attr, *bp, *p, *result, *buff, *text;
	int i, j, aflags, ok;
	dbref aowner, obj;
	ATTR *atr;
	char *cargs[NUM_ENV_VARS];
	...
	queue_string(d, "HTTP_BEGIN");
	queue_string(d, "\r\n");

	if (!arg || !*arg) {
		queue_string(d, "<H3>You must specify an object.</H3>");
		queue_string(d, "\r\n");
		queue_string(d, "HTTP_END");
		queue_string(d, "\r\n");
		return;
	}
	for (i = 0; i < NUM_ENV_VARS; i++)
	...
	if (!object || !isdigit(*object)) {
		queue_string(d, "<H3>You must specify a valid object.</H3>");
		queue_string(d, "\r\n");
		queue_string(d, "HTTP_END");
		queue_string(d, "\r\n");
		return;
	}
	obj = (dbref) atoi(object);
	...
	if (!attr) {
		...
		atr = atr_str(attr);
		if (!atr) {
			queue_string(d, "<H3>You must specify a valid attribute.</H3>");
			queue_string(d, "\r\n");
			queue_string(d, "HTTP_END");
			queue_string(d, "\r\n");
			return;
	...
	STARTLOG(LOG_ALWAYS, "EXT", "HTTP")
		buff = alloc_lbuf("handle_http.LOG");
	sprintf(buff, "[%d/%s] Http opened to object %d, attribute '%s'",
		d->descriptor, d->addr, (int)obj, atr->name);
	log_text(buff);
	free_lbuf(buff);
	ENDLOG

		if (bp) {
		for (p = (char *)strtok(bp, "="), i = 0;
		     (p != NULL) && (i < NUM_ENV_VARS);
		     p = (char *)strtok(NULL, "="), i++) {
			cargs[i] = alloc_lbuf("handle_http.envvars");
			StringCopy(cargs[i], p);
		}
	} else {
		i = 0;
	}

	result = alloc_lbuf("handle_http.result");
	if (!atr) {
		StringCopy(result, "<H3>Attribute not found.</H3>");
	} else {
		text = atr_get(obj, atr->number, &aowner, &aflags);
		bp = result;
		p = text;
		exec(result, &bp, 0, Owner(obj), Owner(obj),
		     EV_EVAL | EV_STRIP | EV_FCHECK,
		     &p,
		     cargs, i);
		*bp = '\0';
		free_lbuf(text);
	...
	if (!Visual(obj) && !(aflags & AF_VISUAL) && !ok) {
		queue_string(d, "<H3>No permission to get attribute.</H3>");
		queue_string(d, "\r\n");
		queue_string(d, "HTTP_END");
		queue_string(d, "\r\n");
		free_lbuf(result);
	...
	queue_string(d, result);
	queue_string(d, "\r\n");
	queue_string(d, tprintf("<P><HR><CENTER><B>TinyMUX %s patchlevel %d [%s] - lauren@ranger.range.orst.edu</B></CENTER>",
				MUX_VERSION, PATCHLEVEL, MUX_RELEASE_DATE));
	queue_string(d, "\r\n");
	free_lbuf(result);
	queue_string(d, "HTTP_END");
	queue_string(d, "\r\n");
