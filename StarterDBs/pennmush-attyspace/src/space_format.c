...
char *format_s_Beam (int x, int a)
{
	...
	for (i = 0 ; i < sdb[x].beam.banks ; ++i) {
		...
		if (!sdb[x].blist.active[i]) {
			strncat(buffer, tprintf("%5.5s   %10.10s/",
			  unparse_arc(sdb[x].blist.arcs[i]),
			  unparse_power(sdb[x].blist.cost[i] / (double) beam_refresh * sdb[x].tech.firing)), sizeof(buffer) - 1);
			...
			if (c == SENSOR_FAIL) {
				...
				strncat(buffer, tprintf("%5.5s - Lock: %-15.15s %10.10s\n",
			 	  unparse_arc(sdb[x].blist.arcs[i]),
			 	  unparse_identity(a, sdb[x].blist.lock[i]),
			 	  unparse_distance(range)), sizeof(buffer) - 1);
			} else if (sdb[x].blist.load[i] + ((double) beam_refresh / sdb[x].tech.firing) > time(NULL)) {
				strncat(buffer, tprintf("%5.5s + Lock: %-15.15s %10.10s\n",
			 	  unparse_arc(sdb[x].blist.arcs[i]),
			 	  unparse_identity(a, sdb[x].blist.lock[i]),
			 	  unparse_distance(range)), sizeof(buffer) - 1);
	...
	for (i = 0 ; i < sdb[x].missile.tubes ; ++i) {
		...
		if (!sdb[x].mlist.active[i]) {
			strncat(buffer, tprintf("%5.5s   %10.10s/",
			  ...
			  unparse_power(sdb[x].mlist.cost[i] / (double) missile_refresh * sdb[x].tech.firing)), sizeof(buffer) - 1);
			strncat(buffer, tprintf("%-10.10s %10.10s\n",
			...
			if (c == SENSOR_FAIL) {
				...
				if (arc_check(c_arc, sdb[x].mlist.arcs[i]) == ARC_FAIL) {
					strncat(buffer, tprintf("%5.5s - Lock: %-15.15s %10.10s\n",
				 	  ...
				 	  unparse_distance(range)), sizeof(buffer) - 1);
				} else if (sdb[x].mlist.load[i] + ((double) missile_refresh / sdb[x].tech.firing) > time(NULL)) {
					strncat(buffer, tprintf("%5.5s + Lock: %-15.15s %10.10s\n",
				 	  unparse_arc(sdb[x].mlist.arcs[i]),
				 	  unparse_identity(a, sdb[x].mlist.lock[i]),
				 	  unparse_distance(range)), sizeof(buffer) - 1);
