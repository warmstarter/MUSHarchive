...
static int qparse_dbref(const char *s);
...
FUNCTION(fun_iter)
{
  ...
  if (!outsep || !list)
    panic("Unable to allocate memory in fun_iter");
  if (nargs < 4)
    strcpy(outsep, " ");
  else {
 ...
 */
  ...
  for (i = 0; i < nqregs; i++) {
    ...
    if (subpatterns < 0)
      ...
      pcre_copy_substring(args[0], offsets, subpatterns, i, renv[curq],
			  BUFFER_LEN);
  }
  mush_free((Malloc_t) re, "pcre");
}
...
static int
regrep_helper(who, what, name, atr, args)
    dbref who __attribute__ ((__unused__));
