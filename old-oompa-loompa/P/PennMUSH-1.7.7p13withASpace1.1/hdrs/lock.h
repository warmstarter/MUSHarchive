...
struct lock_list {
  ...
  struct boolexp *key;		/**< Lock value ("key") */
...
typedef struct lock_info LOCKINFO;
/** A lock.
...
#define LF_WIZARD  0x4		/* Only wizards can set/unset this lock */
...
/* lock.c */
struct boolexp *getlock(dbref thing, lock_type type);
struct boolexp *getlock_noparent(dbref thing, lock_type type);
