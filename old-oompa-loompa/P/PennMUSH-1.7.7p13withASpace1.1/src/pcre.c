...
match_ref(int offset, register const uschar * eptr, int length,
	  volatile match_data * md, unsigned long int ims)
{
  const uschar *p = md->start_subject + md->offset_vector[offset];

#ifdef BACKTRACK_LIMIT
  if (backtrack_count++ > BACKTRACK_LIMIT || cpu_time_limit_hit)
    longjmp(backtrack_jmp, 1);
#endif
...
match(register const uschar * eptr, register const uschar * ecode,
      int offset_top, volatile match_data * md, unsigned long int ims,
