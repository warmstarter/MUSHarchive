 ...
 *	Mods for DragonMud, especially visible exits, and
 *	TinyMUCK (Brigadoon).
 *
 * 27-Sep-90  Michael Mauldin (mlm) at Carnegie-Mellon University
 *	Tenth ceremonial relase.
 *	Mods for TinyHELL.
 *
...
read_map (fn)
char *fn;
{ char buf[BIGBUF], name[MSGSIZ], dir[MSGSIZ], wname[MSGSIZ];
  register char *s;
  register O_EXIT *xp;
  FILE *mfile;
  ...
  { fclose (mfile);
    fatal ("Fatal, '%s' %s\nBad line: %s\n",
	   fn, "does not have a valid Maas-Neotek map file header", buf);
  }
  ...
  { buf[strlen (buf) - 1] = '\0';
    ...
    { case 'K':	nrooms = atol (buf+2);
      ...
      case 'F':	if (cur >= 0)
		{ if (sscanf (buf, "F:%ld %ld %ld %ld %ld %ld %ld %ld %ld",
			      &room[cur].number, &room[cur].firstin,
			      &room[cur].lastin, &room[cur].cntin,
			      &room[cur].totalin, &room[cur].awakesum,
			      &room[cur].sleepsum, &room[cur].msgsum,
			      &room[cur].flags) == 9)
		  { /* good, new room with flags */ }

		  /* Old style rooms didnt save flags in map file */
		  else if (sscanf (buf, "F:%ld %ld %ld %ld %ld %ld %ld %ld",
			      &room[cur].number, &room[cur].firstin,
			      &room[cur].lastin, &room[cur].cntin,
			      &room[cur].totalin, &room[cur].awakesum,
      ...
      case 'X':	if (cur >= 0 && buf[2])
		{ room[cur].exlist = makestring (buf + 2); }
		else
		{ fprintf (stderr, "Warn: cur %ld, ignoring '%s'\n", cur,buf);}
		break;
...
{ register long i;
  ...
  { if (room[i].name && (!reach_only || can_reach (i)))
    { fprintf (outfil, "\nR:%ld %s\n", i, room[i].name);
      ...
      fprintf (outfil, "F:%ld %ld %ld %ld %ld %ld %ld %ld %ld\n",
	       room[i].number, room[i].firstin, room[i].lastin,
	       room[i].cntin, room[i].totalin, room[i].awakesum,
	       room[i].sleepsum, room[i].msgsum, room[i].flags);
      ...
      if (room[i].exlist && room[i].exlist[0])
      { strncpy (buf, room[i].exlist, BUFSIZ);
        buf[BUFSIZ-3] = '\0';
        fprintf (outfil, "X:%s\n", buf);
      }
...
{ register long i;
  ...
  { if (!streq (room[best].name, room1))
    { if (!ismuck || *room1 != '@' && room[best].name[0] != '@')
      { fprintf (stderr, "Warn: Changing room name '%s' to '%s' (%ld)\n",
		  room[best].name, room1, best);
      }
      else
      { fprintf (stderr,
		 "Warn: Changing room name '%20.20s' to '%20.20s' (%ld)\n",
		 room[best].name, room1, best);
      }
...
long close_room (roomnam)
char *roomnam;
{ register long i, anchor, result = -1;
  char pat[TOKSIZ], dat[TOKSIZ];
  int ptype = 0, tmpdebug = 0;

  if (debug)
  { fprintf (stderr, "Find: close room(%s) called\n", roomnam);
    debug--; tmpdebug++;
  }
  ...
  if (result < 0 &&
      (streq (roomnam, "here") ||
       ...
       streq (roomnam, "aqui") ||
       streq (roomnam, "you") ||
       streq (roomnam, "este cuarto") ||
       streq (roomnam, "este sala") ||
       streq (roomnam, "este sitio") ||
       streq (roomnam, "where you are") ||
       streq (roomnam, "this place") ||
       streq (roomnam, "this spot") ||
       streq (roomnam, "this room")))
  { result = hererm; ptype=1; }

  /*----Special for 'First and Main' in brigadoon----*/
  if (result < 0 &&
      streq (world, "Brigadoon") &&
      (streq (roomnam, "town") ||
       streq (roomnam, "town square") ||
       ((sindex (roomnam, "first") || sindex (roomnam, "1st")) &&
        (sindex (roomnam, "and") || sindex (roomnam, "&")) &&
        sindex (roomnam, "main"))) &&
      (i = find_room ("Intersection of Main and 1st", NULL)) >= 0)
  { result = i; ptype=2; }

  /*----Special for 'Tanstaafl Center' in brigadoon----*/
  if (result < 0 &&
      streq (world, "HoloMuck") &&
      (streq (roomnam, "town") ||
       streq (roomnam, "center") ||
       streq (roomnam, "town center") ||
       streq (roomnam, "town square")) &&
      (i = find_room ("Tanstaafl Center", NULL)) >= 0)
  { result = i; ptype=2; }

  /*----Special for 'town' (assumes 'town' from robots home goes to town)----*/
  if (result < 0 &&
      (streq (roomnam, "town") ||
       streq (roomnam, "town square") ||
       streq (roomnam, "the town square")) &&
      (((anchor = homerm) >= 0 &&
	(i = leads_to (anchor, "town")) >= 0 &&
	 stlmatch (room[i].name, "Town")) ||
       (streq (world, "Islandia") && 
	(i = find_room ("Town Square", NULL)) >= 0)))
  { result = i; ptype=2; }
  ...
  if (result < 0 &&
      ...
      ((i = find_room ("Library Vestibule", NULL)) >= 0 ||
       (i = find_room ("Library lobby", NULL)) >= 0 ||
       (i = find_room ("Reference Room", NULL)) >= 0))
  { result = i; ptype=4; }

  /*---- Special for 'rec room' ----*/
  if (result < 0 && recrm >= 0 &&
      (streq (roomnam, "the rec room") ||
       streq (roomnam, "the recroom") ||
       streq (roomnam, "rec room") ||
       streq (roomnam, "recroom")))
  { result = recrm; ptype=5; }

  /*---- Special for 'time traveller nexus' ----*/
  if (result < 0 && 
      (streq (roomnam, "the main nexus") ||
       streq (roomnam, "the nexus") ||
       streq (roomnam, "main nexus") ||
       streq (roomnam, "nexus")) &&
      (i = find_room ("The Inter Nexus", NULL)) >= 0)
  { result = i; ptype=6; }
  ...
  if (result < 0 &&
	...
	sindex (roomnam, "source") ||
        sindex (roomnam, "guia") ||
        sindex (roomnam, "instrucciones") ||
        sindex (roomnam, "programa"))) &&
      (i = find_room ("Maas-Neotek Robot User's Guide", NULL)) >= 0)
  { result = i; ptype=9; }

  /* Special for Brigadoon chat room */
  if (result < 0 &&
      sindex (roomnam, "inter nexus") &&
      sindex (roomnam, "chat") &&
      sindex (roomnam, "room") &&
      !sindex (roomnam, "line") &&
      (i = find_room ("The Chat Line [1028]", NULL)) >= 0)
  { result = i; ptype = 10; }
  ...
  /*---- Return result ----*/  
  if (result >= 0 && debug)
  { fprintf (stderr, "Map:  close_room maps '%s' to '%s', match type %d\n",
	     roomnam, room[result].name, ptype);
  }
  else if (debug)
  { fprintf (stderr, "Find: closeroom(%s) fails\n", roomnam); }
 ...
 * 
 * Note: future plan is to keep list sorted by destination room number,
 *	 this will allow us to print the answer to "what exits are here"
 *	 a lot prettier.
 ****************************************************************/
...
{ register O_EXIT *xp;
  ...
  if (to == from && awake && debug)
  { fprintf (stderr, "Exit: marking '%s' from %s(%d) as a NOP\n",
	     dir, room_name(from), from);
  }
...
# endif
    ...
    { if (room[i].name && path[i].depth < 0)
      { lastxp = NULL;
        ...
        for (xp = room[i].exits; xp && !found; lastxp = xp, xp = xp->next)
...
# endif
    ...
    if (!found && (desc == NEXTMOVEHOME || desc == SHORTPATHHOME) &&
	homerm >= 0 && path[homerm].depth >= 0)
    { extended++;
      path[from].dir = "home";
      path[from].loc = homerm;
      path[from].depth = path[homerm].depth + 1;
      found++;
      if (debug)
      { fprintf (stderr, "Path: using home, depth %d\n",
		 path[from].depth);
      }
    }
  }
  ...
  if (found && desc != NEXTMOVE && desc != NEXTMOVEHOME)
  { 
    *buf = '\0';
...
long unexplored_room ()
{ register long rm;
  static long lastreset = 0;
  register char *dir;
  static long last_cleared = 0;
  ...
  /* Occasionally reset the explored bits */
  if (last_cleared + 8 * HOURS < now)
  { for (rm = 0; rm < rooms; rm++)
    { ROOM_CLR (rm, RM_EXPLORED); }
    lastrm = 0;
    last_cleared = now;
    fprintf (stderr, "Expl: clearing explored markers from all %d rooms\n",
	     rooms);

    /* Recurse: time in last_cleared prevents infinite recursion */
 ...
 * is true, chose randomly from all unexplored exits in the room.
 ****************************************************************/
...
{ register long i, cnt=0;
  ...
  register O_EXIT *xp;
  long type = 0;
  char buf[BIGBUF+BIGBUF];

  if (rm < 0) return (NULL);

  if (ROOM_GET (rm, RM_EXPLORED)) return (NULL);
  ...
  if (usedesc &&
      (room[rm].desc && room[rm].desc[0] ||
       room[rm].exlist && room[rm].exlist[0]) ||
       room[rm].contents && room[rm].contents[0])
  { strcpy (buf, "");

    if (room[rm].desc) strcpy (buf, lcstr (room[rm].desc));

    if (room[rm].exlist)
    { if (*buf) strcat (buf, ", ");
      strcat (buf, room[rm].exlist);
    }

    if (room[rm].contents)
    { if (*buf) strcat (buf, ", ");
      strcat (buf, room[rm].contents);
    }
  ...
  /* Ah, this room has been explored...reclaim its bad exits */
  if (dir == NULL && !ROOM_GET (rm, RM_EXPLORED)) clean_bad_exits (rm);

  if (debug)
  { fprintf (stderr, "Expl: unexp_exit type %ld '%s'(%ld) => <%s>\n",
		type, room_name (rm), rm, dir ? dir : "NULL");
  }
...
/****************************************************************
 * clean_bad_exits: This room has been completely explored.
 *	Remove all exits from the exit list that do not go
 *	to other rooms.
 ****************************************************************/

clean_bad_exits (rm)
int rm;
{ register O_EXIT *ep, *tp, *lastep = NULL;
  register int total=0, cnt=0;

  if (rm < 0) return;

  /* Loop through each exit, removing bad ones */
  for (ep = room[rm].exits; ep;)
  { if (ep->room == rm)
    { cnt++;
      ...
      if (lastep == NULL)	{ room[rm].exits = tp; }
      else			{ lastep->next = tp; }

      /* Then reclaim the space */
      ep->next = NULL;	/* Just in case anybody is still pointing here */
      free (ep);
      exit_sp -= sizeof (O_EXIT);
      exit_ct --;

      /* And loop to the next exit in the list */
      ep = tp;
    }
  ...
  if (terse < 2)
  { fprintf (stderr,
	     "Expl: Marking %s(%d) explored, reclaimed %d of %d exits\n",
	     room_name (rm), rm, cnt, total);
  }

  ROOM_SET (rm, RM_EXPLORED);
  newexits++;
}
...
{ long oldplace;
  ...
  { 
    ...
    if (hererm != homerm &&
        (now - room[homerm].lastin) > (randint (20) + 20) * MINUTES)
    { if (!terse)
      { fprintf (stderr, "Expl: getting homesick after %ld seconds\n",
			   now - room[homerm].lastin);
      }
      reset_robot ("homesick");
      return (1);
    }
    ...
    if (randint (100) < 66)
    { 
      /* If no unexplored room, fail, and we fall through to do_twiddle */
      ...
      if (eroom != hererm &&
	  (find_path (hererm, eroom, NEXTMOVEHOME) ||
	   homerm == eroom ||
	...
	if (doing_works)
	{ command ("@doing exploring %s", room_name (pagedto));
	  lastexpl = pagedto;
	}
	...
	else if ((new_goal || !terse) && terse < 2)
    ...
    { if (!terse)
      ...
      if (lastexpl != hererm && doing_works)
      { command ("@doing exploring %s", room_name (hererm)); 
      	lastexpl = -1;
      }
    ...
    { fprintf (stderr, "Expl: stuck in '%s'(%ld), going home\n",
      ...
      movemud ("straighthome"); hangaround (speed); return (1);
    }
...
/****************************************************************
 * do_findold: Check on old rooms
 ****************************************************************/

do_findold (age)
long age;
{ long oldest = 0, lastin = now, rm;
  static long oldest_age = 0;

  if (!visitold) return (0);

  /* Can we shortcut the work? */
  if (now - oldest_age <= age) return (0);

  /* Check more important goals first */
  if (!nextwait && ! *pagedby && ! *typecmd && !dead &&
      !takingnotes && !playinghearts)
  {
    /* Find oldest reachable room */
    for (rm=0; rm<rooms; rm++)
    { if (room[rm].name &&
	  room[rm].lastin < lastin &&
	  can_reach (rm))
      { oldest = rm;
        lastin = room[rm].lastin;
	oldest_age = lastin;
      }
    }

    /* Now if that room is older than threshold, set a page to it */
    if ((now-lastin) > age)
    { strcpy (pagedby, "<EXPLORING>");
      pagedfrom = hererm;
      pagedto = oldest;
      ...
      { fprintf (stderr,
		 ...
		 ctime (&pagedat) + 4,
		 lastin ? exact_dur (now-lastin) : "forever",
		 room[pagedto].name, pagedto);
      }
      else if (terse < 2)
      { fprintf (stderr, "Expl: %15.15s, find old room (%s) %s(%ld)\n",
		 ctime (&pagedat) + 4,
		 lastin ? exact_dur (now-lastin) : "forever",
		 room[pagedto].name, pagedto);
      }
...
{ long pl;
  ...
  /* Currently in conv? */
  if (((now = time (0)) < speaktime + 60) &&
      *speaker && (pl = find_player (speaker)) >= 0 &&
      player[pl].active && player[pl].present)
  { in_conv = 1; }

  /* Must disengage from a conversation */    
  if (dest && in_conv && msgtype <= M_WHISPER)
  { switch (nrrint (184, 4))
  ...
  /* Must disengage from a conversation */    
  else if (in_conv && msgtype <= M_WHISPER)
  { switch (nrrint (186, 4))
...
/****************************************************************
 * ispublic: A list of 'public' places 
 ****************************************************************/
...
char *pubrooms[] = {
  ...
  "North Hall",
  "Front Lawn",
  "West End Institute for the Reality Disabled",
  "Aspnes Park",
  "Mail Room",
  "Play Room",
  NULL };
...
{ register char **rp;
  ...
  for (rp=pubrooms; *rp; rp++)
  { if (stlmatch (loc, *rp)) return (1); }

  return (0);
}

/****************************************************************
 * is_noisy: A list of 'noisy' places 
 ****************************************************************/

char *noiserooms[] = {
  "Pandemonium Town Square", "Purgatory", "The Great Abyss",
  "The Nexus Club", "The Town Square", "Town Square", NULL
};
...
{ register long rm, i;
  ...
  { reply ("\"I have never heard of %s{, n}.", roomnam);
    return;
  }
  ...
  if (!printed)
  { reply ("\"I don't know any exits to %s{, n}.", room_name (rm)); }
  else
  { reply ("|done."); }
}
...
{ register long rm, i;
  ...
  if (!printed)
  { reply ("\"I don't know any exits from %s{, n}.", room_name (rm)); }
  else
  { reply ("|done."); }
}
...
{ register long rm;
  ...
  { for (rm = 0; rm < rooms; rm++)
    { if (room[rm].name && sindex (lcstr (room[rm].name), pat))
      { if (!printed++)
        { reply ("\"Here are the rooms that match '%s'{, n}:", pat); }
      ...
      { if (!printed++)
        { reply ("\"Here are the rooms whose descriptions match '%s', %s:",
		 pat, name); }
          ...
          if (printed > 10 && alone > 2 ||
	      printed > 20 && !isowner (name))
          { reply ("\"There are more rooms, but %s, %s.",
	           " it's too crowded here to go on", name);
	    break;
          }
...
/****************************************************************
 * most_rm_query: Which room(s) have the most of something
 ****************************************************************/

# define QCNT 10

most_rm_query (name, type)
char *name;
int type;
{ long rm, b, i, j, best[QCNT], bcnt = 0;
  double val, bestval[QCNT];
  
  
  /* Search through every room */
  for (rm=0; rm<rooms; rm++)
   { if (room[rm].cntin < 4 || room[rm].totalin < 30) continue;
     if (type == NOISE && room[rm].totalin < 60) continue;
     if ((now - room[rm].lastin) > 1 * DAYS) continue;
     if (!can_reach (rm)) continue;

    switch (type)
    { case NOISE:    val = (double) room[rm].msgsum / room[rm].totalin; break;
      case PEOPLE:   val = (double) room[rm].awakesum / room[rm].cntin; break;
      case SLEEPERS: val = (double) room[rm].sleepsum / room[rm].cntin; break;
      case FRIENDLY: val = (double) room[rm].friendly; break;
    }
  ...
  switch (type)
  { case NOISE:		reply ("\"The %d noisiest rooms are:", QCNT);
  			for (b=0; b<bcnt; b++)
			{ strcpy (speaker, name);
			  reply ("| %s averages %1.2lf messages per hour",
				 room_name (best[b]), bestval[b] * HOURS);
			}
			break;
  
    case PEOPLE:	reply ("\"The %d most crowded rooms are:", QCNT);
  			for (b=0; b<bcnt; b++)
			{ strcpy (speaker, name);
			  reply ("| %s averages %1.2lf people",
				 room_name (best[b]), bestval[b]);
			}
			break;
    
    case SLEEPERS:	reply ("\"The %d rooms with the most sleepers are:",
			       QCNT);
  			for (b=0; b<bcnt; b++)
			{ strcpy (speaker, name);
			  reply ("| %s averages %1.2lf sleepers",
				 room_name (best[b]), bestval[b]);
			}
			break;
    
    case FRIENDLY:	reply ("\"The %d friendliest rooms since I logged in:",
			       QCNT);
  			for (b=0; b<bcnt; b++)
			{ strcpy (speaker, name);
			  reply ("| %s has had %1.0lf friendly messages",
				 room_name (best[b]), bestval[b]);
			}
			break;
  }
...
{
  if (!str)
  { crash_robot ("null string in check_id\n"); }
